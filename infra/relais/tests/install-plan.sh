...
verifier "prend Telegram en darwin-arm64, de NOTRE publication" "$RELEASES_ATTENDU/mautrix-telegram-darwin-arm64" "$MAC"
...
verifier "prend Telegram en amd64" "mautrix-telegram-amd64" "$LX"
...
verifier "prend Telegram en arm64" "mautrix-telegram-arm64" "$LA"
...
verifier "déplace Instagram, Messenger, X, Slack et Telegram aussi" "29350 (Instagram), 29351 (Messenger), 29352 (X), 29355 (Slack) et 29337 (Telegram)" "$PORT"
...
echo "Les sept ponts, même traitement"
...
verifier "Telegram passe par la même fonction pont()" "pont telegram " "$SRC"
verifier "le préfixe de commande de Telegram est celui de l'app" "'!tg' telegrambot" "$SRC"
verifier "Telegram reçoit ses clés d'API dans la config du pont" "api_hash:" "$SRC"
verifier "sans clés d'API, Telegram n'est ni configuré ni démarré" "ni configuré ni démarré" "$SRC"
...
verifier "le désinstalleur retire aussi Telegram" "mautrix-telegram" "$DESINSTALL_SRC"
