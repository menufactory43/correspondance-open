...
**2) les réseaux** — 6 chez nous (iMessage, Signal, WhatsApp, Instagram, Messenger, X) contre 14 chez
...
| Partager depuis une autre app (Photos, Safari, Fichiers → une personne) | **iOS seulement** : extension de partage héritée d'Element, gardée par la réécriture 2024, présente dans la rangée des visages de la feuille. **Absente sur Desktop** : Electron ne sait pas fournir d'extension de partage macOS, et le bundle 4.3.73 ne contient aucune chaîne liée au partage (glisser-déposer dans le fil seulement) | **Fait, sur les deux** (2026-09-03) : `Partage/` (la feuille, commune) + cibles `CorrespondanceiOSPartage` et `CorrespondancePartage`, contrat dans `Core/Domain/Partage.swift` (testé). iPhone : l'extension envoie elle-même au Relais avec la session du Trousseau partagé, et dépose dans le conteneur du groupe si le Relais ne répond pas ou si le salon est chiffré ; l'app vide la boîte au premier plan. Mac : l'extension, sandboxée, dépose toujours et réveille l'app par `correspondance://partage` — c'est l'app qui envoie, iMessage compris par Messages. Rangée des visages : `IntentsSupported` + les `INSendMessageIntent` déjà donnés par les notifications, le fil arrive présélectionné. HEIC → JPEG à la copie. Sur Mac, macOS ne coche pas une extension tierce tout seul : une fois dans Réglages Système › Extensions › Partage (bouton dans Réglages › Autorisations, ou `pluginkit -e use -i app.correspondance.Correspondance.Partage`). Prouvé sur Mac le 2026-09-03 (photo + mot partis dans Note à soi par la feuille) ; iOS construit et embarqué, pas encore essayé sur l'iPhone | — | — |
...
API locale/MCP (nos fils pilotables par un agent extérieur — voir § « IA & écosystème ») · ~~Extension de partage iOS et Mac~~ (faite le 2026-09-03, cf. § 4) ·
