...
/// L'inbox de Linux, au-dessus de Core — c'est LE MAGASIN DE L'IPHONE, porté
/// tel quel.
///
/// Linux est, comme l'iPhone, un client Matrix pur : pas d'iMessage, pas de
/// carnet d'adresses, pas de fenêtres détachées. Il tient une boucle `/sync`,
/// une liste de conversations, un fil par conversation ouverte, et l'état de
/// conversation du Relais. Ce fichier est une copie de
/// `CorrespondanceiOS/Store/RelayStore.swift` aux différences de plateforme
/// près (journal, notifications, pas de mode démo ni de push) : quand l'un
/// change, l'autre suit.
///
...
package final class RelayStore {
  ...
  package static func readable(_ error: Error) -> String {
    ...
    if case .transport = error as? MatrixError {
      return String(
        localized: "Le Relais ne répond pas à cette adresse. Vérifie-la, et que Tailscale est connecté.")
    }
  ...
  // Pas de message vocal sous Linux : ni micro AVFoundation, ni encodeur
  // Opus dans un binaire statique. Les vocaux REÇUS se lisent, eux, dans le
  // navigateur.
  ...
  private func forwardablePaths(of message: ChatMessage) -> [String] {
    message.attachments.map(AttachmentRepair.repaired).compactMap { $0.resolvedFileURL?.path }
  }
